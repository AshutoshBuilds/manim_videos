...
def get_y_bounds(t):
    """
    Calculate the y-axis bounds based on the animation time t.
    This ensures consistent bounds for both the y-axis and scatter points.
    """
    # Use predefined y-axis ranges
    initial_y_min = 0.3887
    initial_y_max = 0.3940
    final_y_min = 0.15
    final_y_max = 0.6
    
    # Interpolate between initial and final y bounds based on t
    # Use a smooth interpolation function (sigmoid-like) to avoid sudden jumps
    smooth_t = 1 / (1 + np.exp(-10 * (t - 0.5)))
    
    lower_bound = smooth_t * final_y_min + (1 - smooth_t) * initial_y_min
    upper_bound = smooth_t * final_y_max + (1 - smooth_t) * initial_y_max
    
    return lower_bound, upper_bound

def get_y_axis(t, intial_bounds, final_bounds, position=None):
    """
    Creates a y-axis with bounds that smoothly interpolate between initial and final bounds
    ...
    # Get y-axis bounds using the shared function
    lower_bound, upper_bound = get_y_bounds(t)
...
class P21(InteractiveScene):
    ...
    def construct(self):
        ...
        # Add a third tracker for the losses scatter plot
        losses_tracker = ValueTracker(0.0)
        ...
        self.play(y_zoom_tracker.animate.set_value(1.0), run_time=2)
        self.wait()
        
        # Third animation: add losses scatter plot
        self.add(losses_scatter)
        # self.play(losses_tracker.animate.set_value(1.0), run_time=2)
        self.wait()
