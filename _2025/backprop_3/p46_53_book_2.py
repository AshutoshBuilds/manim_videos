...
class p47h(InteractiveScene):
    def construct(self):
        ...
        outline_4.set_stroke(width=5)
        outline_5.set_stroke(width=5)
        # polygons_31_merged_flat.set_opacity(1.0)
        # self.remove(polygons_31_merged_flat)

        outline_4.move_to(ORIGIN)
        outline_5.move_to(ORIGIN)

        self.add(outline_4)
        self.add(outline_5)
        self.wait()

        #Now some overhead, and full colored polygon. 
        self.frame.reorient(0, 0, 0, (-0.08, -0.06, 0.0), 2.61)
        self.wait()
        self.remove(outline_4, outline_5)

        layer_2_polygons_flat.move_to(ORIGIN)
        layer_2_polygons_flat.set_opacity(0.75)
        self.add(layer_2_polygons_flat)
        self.wait()
        self.remove(layer_2_polygons_flat)

        layer_1_polygons_flat.set_opacity(0.5)
