...
# PRIMARY_COLOR='#5C4F35'
PRIMARY_COLOR='#948979'
YELLOW='#ffd35a'
BLUE='#65c8d0'
# BLUE='00FFFF'
GREEN='#00a14b'
...
PERCENTILES=[99, 99, 95, 97, 99, 98, 99, 98, 98, 99]

# THRESHOLDS=[0.75, 0.45, 0.5, 0.5, 0.35, 0.35, 0.35, 0.4]
# PERCENTILES=[97, 97, 93, 95, 95, 95, 95, 95]


# THRESHOLDS=[0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4] 
# PERCENTILES=[96, 96, 96, 96, 95, 95, 95, 95, 95, 95]
...
# custom_cmap_tans = mcolors.LinearSegmentedColormap.from_list('custom', ['#FFFFFF', PRIMARY_COLOR], N=256)
custom_cmap_tans = mcolors.LinearSegmentedColormap.from_list('custom', ['#FFFFFF', PRIMARY_COLOR], N=256)
custom_cmap_cyan = mcolors.LinearSegmentedColormap.from_list('custom', ['#FFFFFF', '#518c9b'], N=256)
# custom_cmap_cyan = mcolors.LinearSegmentedColormap.from_list('custom', ['#FFFFFF', '#00FFFF'], N=256)
...
def get_mlp(w1, 
    ...
    w2_abs=np.abs(w2)
    w2_scaled=w2_abs/np.percentile(w2_abs, PERCENTILES[1])
    for i, hidden_neuron in enumerate(hidden_layer):
        for j, out_neuron in enumerate(output_layer):
            if np.abs(w2_scaled[i, j])<THRESHOLDS[1]: continue
            if abs(i-j)>6: continue #Let's try just drawing local ones.
    ...
    if grads_1 is not None:
        ...
        grads_1_scaled=grads_1_abs/np.percentile(grads_1_abs, PERCENTILES[2])
        for i, in_neuron in enumerate(input_layer):
            for j, hidden_neuron in enumerate(hidden_layer):
                if np.abs(grads_1_scaled[i, j])<THRESHOLDS[2]: continue
    ...
    if grads_2 is not None:
        ...
        grads_2_scaled=grads_2_abs/np.percentile(grads_2_abs, PERCENTILES[3])
        for i, hidden_neuron in enumerate(hidden_layer):
            for j, out_neuron in enumerate(output_layer):
                if np.abs(grads_2_scaled[i, j])<THRESHOLDS[3]: continue
                if abs(i-j)>6: continue #Need to dial this up or lost it probably, but it is helpful!
                start_point, end_point = get_edge_points(hidden_neuron, out_neuron, NEURON_RADIUS)
                line_grad = Line(start_point, end_point)
...
def get_attention_layer(attn_patterns):
    ...
    attention_border.set_stroke(width=1.0, color=PRIMARY_COLOR)
...
def get_mlp_connections_left(attention_connections_left, mlp_out, connection_points_left, attention_connections_left_grad=None):
    ...
    attention_connections_left_scaled=attention_connections_left_abs/np.percentile(attention_connections_left_abs, PERCENTILES[4])
    for i, mlp_out_neuron in enumerate(mlp_out):
        for j, attention_neuron in enumerate(connection_points_left):
            if np.abs(attention_connections_left_scaled[i, j])<THRESHOLDS[4]: continue
    ...
    if attention_connections_left_grad is not None: 
        ...
        attention_connections_left_grad_scaled=attention_connections_left_grad_abs/np.percentile(attention_connections_left_grad_abs, PERCENTILES[5]) #np.percentile(attention_connections_left_abs, 99)
        for i, mlp_out_neuron in enumerate(mlp_out):
            for j, attention_neuron in enumerate(connection_points_left):
                if np.abs(attention_connections_left_grad_scaled[i, j])<THRESHOLDS[5]: continue
                if abs(i/4-j)>3: continue #Need to dial this up or lost it probably, but it is helpful!
                start_point, end_point = get_edge_points(mlp_out_neuron, attention_neuron, 0.06)
                line = Line(start_point, attention_neuron.get_center())
                # line.set_stroke(width=1, opacity=0.3)
...
def get_mlp_connections_right(attention_connections_right, mlp_in, connection_points_right, attention_connections_right_grad=None):
    ...
    attention_connections_right_scaled=attention_connections_right_abs/np.percentile(attention_connections_right_abs, PERCENTILES[6])
    for i, attention_neuron in enumerate(connection_points_right):
        for j, mlp_in_neuron in enumerate(mlp_in):
            if np.abs(attention_connections_right_scaled[i, j])<THRESHOLDS[6]: continue
    ...
    if attention_connections_right_grad is not None: 
        ...
        attention_connections_right_grad_scaled=attention_connections_right_grad_abs/np.percentile(attention_connections_right_grad_abs, PERCENTILES[7])
        for i, attention_neuron in enumerate(connection_points_right):
            for j, mlp_in_neuron in enumerate(mlp_in):
                if np.abs(attention_connections_right_grad_scaled[i, j])<THRESHOLDS[7]: continue
                if abs(j/4-i)>3: continue #Need to dial this up or lost it probably, but it is helpful!
                start_point, end_point = get_edge_points(mlp_in_neuron, attention_neuron, 0.06)
                line = Line(start_point, attention_neuron.get_center())
                # line.set_stroke(width=1, opacity=0.3)
    ...
    for i in range(num_input_neurons):
        if i == num_input_neurons//2:  # Middle position for ellipsis
            ...
            if neuron_count in prompt_neuron_indices:
                neuron.set_fill(color=PRIMARY_COLOR, opacity=1.0)
    ...
    for i in range(num_output_neurons):
        if i == num_output_neurons//2:  # Middle position for ellipsis
            ...
            if not empty: 
                ...
                text_color=get_nueron_color(np.clip(snapshot['topk.probs'][neuron_count],0.8, 1.0),vmax=np.max(snapshot['topk.probs'])),
...
class LlamaLearningTwelveE(InteractiveScene):
    def construct(self):
        ...
        unembed_scaled=unembed_abs/np.percentile(unembed_abs, PERCENTILES[8])
        for i, n1 in enumerate(mlps[-1][4]):
            for j, n2 in enumerate(output_layer[0]):
                if np.abs(unembed_scaled[i, j])<THRESHOLDS[8]: continue
        ...
        unembed_scaled_grad=unembed_grad_abs/np.percentile(unembed_grad_abs, PERCENTILES[9])
        for i, n1 in enumerate(mlps[-1][4]):
            for j, n2 in enumerate(output_layer[0]):
                if np.abs(unembed_scaled_grad[i, j])<THRESHOLDS[9]: continue
                if abs(j-i)>8: continue #Need to dial this up or lost it probably, but it is helpful!
                start_point, end_point = get_edge_points(n1, n2, 0.06)
                line = Line(start_point, n2.get_center())
                # line.set_stroke(width=1, opacity=0.3)
        ...
        # all_forward_passes[0].set_opacity(0.4)
